...
func Write(writer io.Writer, application string, item Diagnostic, structured bool) error {
	if structured {
		return json.NewEncoder(writer).Encode(envelope{Diagnostics: []Diagnostic{item}})
	}
	...
	if item.Hint != "" {
		_, err := fmt.Fprintf(writer, "%s: hint: %s\n", escapeControls(application), escapeControls(item.Hint))
		return err
	}
	return nil
}
